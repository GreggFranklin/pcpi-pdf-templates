 ...
 * 3+ => "A, B, and C"
 */
$join_oxford = function (array $items) {
 ...
 *
 * Purpose:
 *   Join an array of strings into a human-readable list using Oxford comma
 *   conventions.
 *
 * Behavior:
 *   []                       → ''
 ...
 * - Trims values
 * - Removes empty items
 * - Does NOT escape output (caller is responsible)
 * - Does NOT add trailing punctuation
 *
 * Usage examples:
 *
 *   $items = ['Petty Theft', 'Burglary', 'Robbery'];
 *   $list  = $oxford_join( $items );
 *   // Result: "Petty Theft, Burglary, and Robbery"
 *
 *   $para( "Applicant reports the following offenses: {$list}." );
 *
 *   // With custom conjunction:
 *   $list = $oxford_join( $items, 'or' );
 *   // "Petty Theft, Burglary, or Robbery"
 *
 * Safe for:
 * - Narrative PDFs
 * - Nested form output
 * - Legal documents (predictable punctuation)
 */
...
/* -------------------------------------------------------------------------
 * LIST FIELD PARSING (non-breaking additions)
 * ------------------------------------------------------------------------ */
/**
 * Return a flat array of non-empty values from a GF List field.
 * Works for:
 * - Serialized array from entry (typical GF List)
 * - Delimited strings (fallback)
 */
$list_field_items = function ($field_id) use ($entry, $val) {
    // Prefer RAW entry value first
    $raw = rgar($entry, (string) $field_id);
    if ($raw === null || $raw === "") {
    ...
    $items = array_values(
        array_unique(array_filter(array_map("trim", (array) $items)))
    );
    return $items;
};
...
$para_other_drugs_list = function (
    ...
    echo "<p>" .
        esc_html($A . " further reports other drugs / last use: " . $list . ".") .
        "</p>";
};
